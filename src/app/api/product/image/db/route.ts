...
export async function POST(req: Request) {
   ...
   if (type == 'thumbnail') {
      res = await Product.findOneAndUpdate({
         ...
         thumbnail: `https://${process.env.LIARA_BUCKET_NAME}.${process.env.LIARA_ENDPOINT}/products/${key}`
      }).exec();
      ...
      (res.images).push(`https://${process.env.LIARA_BUCKET_NAME}.${process.env.LIARA_ENDPOINT}/products/${key}`)
      res.save()
   }
